...
import {CAPACITY,DEPOSITS,LEVEL_BLOCKS,SURVEY_SITES,DURATION,RESOURCES,STEP,terrainHeight,type Kind} from '../content/config.ts';
...
export type Input={x:number;z:number;yaw:number;jump:boolean;burst:boolean;grab:boolean;scan:boolean;use:boolean;sprint:boolean;brace:boolean;jet?:boolean};
...
export class Moon {
  ...
  fuel=100;jetting=false;fuelDelay=0;surveyed=new Set<string>();
  ...
  constructor(){
    ...
    for(const b of LEVEL_BLOCKS)this.world.createCollider(RAPIER.ColliderDesc.cuboid(b.w/2,b.h/2,b.d/2).setTranslation(b.x,b.y,b.z).setFriction(.9).setCollisionGroups(0x0001ffff));
  ...
  get nearShip(){const p=this.player.translation();return Math.hypot(p.x,p.z)<8&&p.y<3&&p.y>-.5;}
  ...
  supportHeight(x:number,z:number,y:number,s:Sample){const hit=this.world.castRay(new RAPIER.Ray({x,y:y+.3,z},{x:0,y:-1,z:0}),20,true,undefined,undefined,s.collider,this.player);return hit?y+.3-hit.timeOfImpact:terrainHeight(x,z);}
  ...
  recall(){this.release();this.player.setTranslation({x:0,y:1.1,z:12},true);this.player.setLinvel({x:0,y:0,z:0},true);this.charges=2;this.fuel=100;this.jetting=false;this.recalls++;this.notify('recall','Suit retrieval complete. Your secured cargo is safe.');}
  finish(){if(this.finished)return;this.release();this.running=false;this.finished=true;this.drilling=false;this.jetting=false;this.notify('finish');}
  step(input:Input){
    ...
    this.jetting=!!input.jet&&this.fuel>.01;
    if(this.jetting){this.fuel=Math.max(0,this.fuel-22*dt);this.fuelDelay=.75;const jv=this.player.linvel(),lift=9*80/(80+mass);this.player.setLinvel({x:jv.x,y:Math.min(6,jv.y+lift*dt),z:jv.z},true);}
    else{this.fuelDelay=Math.max(0,this.fuelDelay-dt);if(this.grounded&&this.fuelDelay===0&&!input.jet)this.fuel=Math.min(100,this.fuel+18*dt);}
    // The ceiling limits upward thrust, never teleports or stops a fall.
    if(p.y>35&&this.player.linvel().y>0){const cv=this.player.linvel();this.player.setLinvel({x:cv.x,y:0,z:cv.z},true);}
    ...
    const v=this.player.linvel(),ax=this.grounded?16:input.brace?7:this.jetting?6:2.5,max=this.grounded?speed:this.jetting?7*carrySpeed(mass):Math.min(4,speed), change=(t:number,c:number)=>clamp(t-c,-ax*dt,ax*dt);
    ...
    if(this.held){const s=this.held,a=s.body.translation(),cv=s.body.linvel(),pv=this.player.linvel(),r=RESOURCES[s.kind],target={x:p.x+Math.sin(input.yaw)*1.7,y:input.brace&&this.grounded?this.supportHeight(p.x+Math.sin(input.yaw)*1.7,p.z-Math.cos(input.yaw)*1.7,p.y,s)+r.radius+.12:p.y+.6,z:p.z-Math.cos(input.yaw)*1.7};
    ...
    this.world.step();
    const surveyPosition=this.player.translation();
    for(const site of SURVEY_SITES)if(this.grounded&&!this.surveyed.has(site.name)&&Math.hypot(surveyPosition.x-site.x,surveyPosition.z-site.z)<3&&Math.abs(surveyPosition.y-(site.y+.84))<1.8){this.surveyed.add(site.name);this.notify('bank',`${site.name} surveyed. ${this.surveyed.size}/2 relays logged.`);}
